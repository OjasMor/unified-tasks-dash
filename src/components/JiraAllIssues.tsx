...
const getPriorityColor = (priority: string | null) => {
  if (!priority || priority === "None") return "bg-muted text-muted-foreground";
...
export const JiraAllIssues = () => {
  ...
  const loadAllIssues = async () => {
    try {
      ...
      // For demo purposes, we'll use the same getMyIssues but could extend to getAllProjectIssues
      const { data, error } = await supabase.functions.invoke('jira-integration', {
        body: { 
          action: 'getMyIssues', // In real implementation, this would be getAllProjectIssues
      ...
      // Mock additional issues for demo
      const mockIssues = [
        {
          id: "demo-1",
          issue_key: "OPS-3",
          ...
          priority_name: "None",
          assignee_display_name: "parshvajain0317",
          project_name: "Productivity Pitstop",
          due_date: null,
          issue_url: "#",
          updated_at_jira: new Date().toISOString()
        },
        {
          id: "demo-2",
          issue_key: "OPS-8",
          summary: "Add Zoom widget",
          status_name: "To Do",
          priority_name: "Medium",
          assignee_display_name: "Ojas Mor",
          project_name: "Productivity Pitstop",
        ...
        },
        {
          ...
          status_name: "In Progress",
          priority_name: "Medium",
          assignee_display_name: "Dhanraj Vedanth",
          project_name: "Productivity Pitstop",
          due_date: new Date(Date.now() + 518400000).toISOString(), // 6 days
          issue_url: "#",
          updated_at_jira: new Date().toISOString()
        },
        {
      ...
      setIssues([...mockIssues, ...(data.issues || [])]);
    } catch (error) {
      console.error('Error loading all Jira issues:', error);
      toast({
        title: "Error",
        description: "Failed to load project issues.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      toast({
        title: "Refreshed",
        description: "Project issues updated.",
      });
    } catch (error) {
        ...
        <div className="space-y-3 h-full overflow-y-auto">
          {issues.map((issue) => {
            ...
            return (
              <div
                key={issue.id}
                className="bg-white border border-gray-200 rounded-lg p-4 cursor-pointer hover:bg-gray-50 transition-colors relative"
                onClick={() => issue.issue_url && issue.issue_url !== "#" && window.open(issue.issue_url, '_blank')}
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {issue.priority_name && issue.priority_name !== "None" && (
                      <Badge 
